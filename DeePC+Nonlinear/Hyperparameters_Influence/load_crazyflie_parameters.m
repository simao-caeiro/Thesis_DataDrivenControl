function param = load_crazyflie_parameters(i_aux)

%% DEEPC PARAMETERS %%

param.incremental_model = false;
...
%         0 0 0];
% 
% aux_B = [0;0;-1/param.nrotor_vehicle_mass_true];
% 
% param.Ac = [zeros(3,3) eye(3) zeros(3);
%     zeros(3,3) zeros(3,3) aux_A;
%     zeros(3,9)];
% 
% param.Bc = [zeros(3,4);
%     aux_B zeros(3,3);
...
% Qx = [0.01; 0.1; 1; 2; 10; 100];
% param.Q = Qx(i_aux)*diag([40, 40, 500, 0, 0, 0, 0, 0, 40]); % i_aux=1:6

param.Q = diag([40, 40, 500, 0, 0, 0, 0, 0, 40]);
param.R = diag([0.5, 20, 20, 2]);

% Rx = [0.01; 0.1; 1; 10; 100];
% param.R = Rx(i_aux)*diag([0.5, 20, 20, 2]); % i_aux=1:5
...
if(param.incremental_model)
    param.output_min = param.Cd * [position_min; velocity_min; angle_min;position_min;-pi/6];
    param.output_min(isnan(param.output_min)) = -inf;
    param.output_max = param.Cd * [position_max; velocity_max; angle_max;position_max;pi/6];
